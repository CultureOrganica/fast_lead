...
2. **Сбор контактных данных**
   ...
   - Выбирает канал: web, WhatsApp, Telegram, Instagram, MAX (для РФ)
...
4. **Маршрутизация по каналу**
   - Оркестратор создает запись лида, валидирует телефон
...
- Telegram и WhatsApp коннекторы
...
3. Cal.com доступен и настроен с валидным API ключом
4. Существующая инфраструктура поддерживает Postgres и Redis
5. CDN настроен и доступен для хостинга статических файлов виджета
6. Клиенты готовы вставить сниппет виджета на свой сайт
7. Объем трафика в MVP не превысит 1000 лидов/час на tenant
8. Операторы инбокса обучены работе с Chatwoot
...
6. Какой fallback при недоступности всех каналов (email, SMS)?
7. Требуется ли поддержка нескольких языков в первой версии виджета или достаточно одного?
8. Какой минимальный набор UTM-меток требуется отслеживать?

## Acceptance criteria

### Измеримые условия приемки для happy path

1. Виджет подключается одной строкой, открывается и собирает имя и телефон
...
5. Telegram поток работает через deeplink, бот отвечает и предлагает слот
6. WhatsApp поток отправляет шаблон только при наличии согласия
7. Все фичи выключаемы флагом без перезапуска сервиса
8. Автоподхват формы работает на тестовом сайте
9. При ошибке канала происходит fallback на web-чат
10. Логи содержат все ключевые события (open, submit, lead_created, booking_confirmed)
...
9. Повторить для Telegram и WhatsApp
