...
- VK провайдер (VK Bots API для отправки сообщений)
...
2. **Сбор контактных данных**
   ...
   - Выбирает канал: web, WhatsApp, Telegram, SMS, Email, VK, Instagram, MAX (для РФ)
   - При выборе VK вводит VK ID или ссылку на профиль
...
4. **Маршрутизация по каналу**
   - Оркестратор создает запись лида, валидирует телефон, email или VK ID (в зависимости от канала)
   - Запускает сценарий канала:
     ...
     - **VK**: отправляется личное сообщение через VK Bots API с приветствием и кнопкой бронирования, опционально VK Mini App для интерактивной записи
...
6. **Коннекторы каналов волной**
   ...
   - VK: интеграция с VK Bots API, отправка личных сообщений, клавиатура с кнопками
...
- Коннекторы каналов: SMS, Email, VK, Telegram, WhatsApp, MAX (РФ)
...
10. Для VK: создавать отдельную группу/сообщество для каждого tenant или использовать одно на всех с разными ботами?
...
7. VK поток отправляет личное сообщение с кнопкой бронирования
8. Telegram поток работает через deeplink, бот отвечает и предлагает слот
9. WhatsApp поток отправляет шаблон только при наличии согласия
10. Все фичи выключаемы флагом без перезапуска сервиса
11. Автоподхват формы работает на тестовом сайте
12. При ошибке канала происходит fallback на web-чат
13. Логи содержат все ключевые события (open, submit, lead_created, booking_confirmed)
...
**Сценарий 4: VK**
1. Ввести VK ID, выбрать канал "VK"
2. Проверить что сообщение пришло в VK
3. Кликнуть на кнопку в сообщении
4. Забронировать слот

**Сценарий 5: Telegram и WhatsApp**
